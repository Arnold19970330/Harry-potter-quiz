...
export function PlayPage() {
  ...
  async function loadRandom() {
    ...
    try {
      ...
      const res = await fetch(`${API}/questions/by-movie/${movieId}`);
      const data = await res.json();
      setQ(data);
    } finally {
      setLoading(false);
  ...
  async function checkAnswer() {
    ...
    const res = await fetch(`${API}/game/check`, {
      ...
      body: JSON.stringify({ id: q.id, answerIndex: selected, gameId: gameId }),
    });
    const data = await res.json();

    setScore(data.stats);
    checkAttempts(data.stats.total);
    setResult(data);
  }

  function checkAttempts (total: number)  {
    if(total == 10) {
      ...
      <div className="mx-auto max-w-2xl p-6 space-y-6">
        <header className="flex items-center justify-between">
          ...
          <p className="text-sm text-slate-500">
            Játékos: <strong>{playerName}</strong>
          </p>
        </header>
                ...
                {!result ? (
                  ...
                  <>
                    ...
                    >
                      {result.correct
                        ? "Helyes! 🎉"
                        : `Helytelen. Helyes válasz: ${result.correctAnswer}`}
                    </span>
                    <button
                      onClick={loadRandom}
